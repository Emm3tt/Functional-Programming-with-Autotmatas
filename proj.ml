...
let a2 = ("q0", [("q0",'a',"q1"); ("q1",'b',"q2");("q2",'c',"q1")], ["q1"])
...
let is_det (start, f, fs) =
  let rec is_det_aux xs = 
    match xs with
    | [] -> true
    | ((q1,s,q2)::xs) -> (List.length (next f q1 s) <= 1) && is_det_aux xs
    in is_det_aux f

  let valid (start,f,fs) = 
    let st = states f
    in List.mem start st && included_in fs st && is_det (start,f,fs)

  let rec apply f (q:string) (s:char):string =
    match f with
    | [] -> failwith "Func cannot be applied"
    | ((q1,s1,q2)::xs) ->
    if (q1=q && s1=s) then q2 else apply xs q s
